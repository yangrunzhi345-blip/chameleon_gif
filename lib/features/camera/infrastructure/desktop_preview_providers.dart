...
/// 桌面相机流预览会话(autoDispose FutureProvider)。
///
/// 生命周期收敛于此:进入拍摄页 watch → startPreview(推流进程 + UDP
/// 地址);页面销毁(autoDispose 无监听)→ onDispose → stopPreview
/// (State.dispose 内不可用 ref,provider 层收敛与
/// cameraControllerProvider 同型)。
///
/// 返回 null = 不适用(Android 取景经插件 surface)/无设备/启动失败,
/// UI 走盲拍兜底(docs/18 D4 回放确认)。
final desktopPreviewUrlProvider = FutureProvider.autoDispose<String?>((
  ref,
) async {
