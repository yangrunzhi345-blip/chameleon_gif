...
/// [FFmpegKit.executeWithArgumentsAsync] 签名镜像(位置可选参数;静态
/// tear-off 为常量,可作默认值;单测注入 fake 替代不可 mock 的静态 API)。
///
/// ⚠️ 必须走参数数组版:字符串版 executeAsync 内部经
/// `FFmpegKitConfig.parseArguments` 按**空格拆分**,输入/输出路径含空格
/// 会被拆裂 → ffmpeg 报 `No such file or directory`(Android 真机图片
/// 模式"源文件不存在或已被移动"根因,2026-08-07 实证);参数数组直传
/// 原生侧,路径任意字符安全。
typedef KitExecuteArgumentsAsync =
...
/// Android 实例化。
///
/// 命令以**参数数组**直传执行(Kit 契约;字符串版按空格拆分,路径含
/// 空格会被拆裂,见 [KitExecuteArgumentsAsync] 注释);进度经 statistics
/// 回调合成 `-progress` 风格文本行喂上层 [ProgressParser](与桌面解析
/// 100% 复用,time 单位微秒与桌面 out_time_us 一致);取消经
/// `FFmpegKit.cancel(sessionId)`(CancelToken 已取消时 onCancel 立即执行,
/// 幂等)。
class FfmpegKitEngine implements FFmpegEngine {
  FfmpegKitEngine({
    this.executeArgumentsAsync = FFmpegKit.executeWithArgumentsAsync,
    this.cancel = FFmpegKit.cancel,
    this.timeout = kKitCommandTimeout,
  });

  final KitExecuteArgumentsAsync executeArgumentsAsync;
  ...
  Future<ConvertResult> convert(
    ...
    final arguments = assembleArguments(request.command);

    final session = await executeArgumentsAsync(
      arguments,
      (s) async {
  ...
  /// 装配 Kit 契约参数列表(纯函数,单测锁定)。
  ///
  /// ⚠️ 返回**参数数组**直传 `executeWithArgumentsAsync`,不拼字符串:
  /// 字符串命令经 `FFmpegKitConfig.parseArguments` 按空格拆分,路径含
  /// 空格会被拆裂(Android 真机"源文件不存在或已被移动"根因);参数数组
  /// 直传原生侧,输入/输出路径任意字符安全。桌面 Process 引擎本就
  /// 参数数组传参,两引擎行为对齐。
  ///
  /// ⚠️ 不含 `ffmpeg` 可执行名前缀:ffmpeg-kit 的 executeWithArgumentsAsync
  /// 直接执行 ffmpeg 本体,带前缀时 `ffmpeg` 被当作输出文件名,真机实测
  /// 报 `Unable to choose an output format for 'ffmpeg'`(桌面 Process 引擎
  /// 需前缀,属引擎差异)。
  ///
  ...
  @visibleForTesting
  static List<String> assembleArguments(List<String> args) {
