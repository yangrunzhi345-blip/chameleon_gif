  ...
  Future<void> _create() {
    return lock.synchronized(() async {
      ...
      final properties = <String, String>{
        ...
        'dither': 'no',
        'cache': 'yes',
        // Chameleon Gif 本地补丁:移除 cache-on-disk。
        // 磁盘缓存要求流可 seek —— 实时预览流(FIFO/流式 http/UDP)均
        // 不可 seek,开启后 mpv 报 "Failed to create file cache" 并致命
        // 退出(No video or audio streams selected),拍摄页实时预览黑屏。
        // 内存缓存(cache=yes)对本地文件与网络流均正常,移除磁盘缓存
        // 不影响现有播放(本地文件播放不依赖磁盘缓存)。
        // 'cache-on-disk': 'yes',
        'hr-seek': 'yes',
